...
public class CoinTower {
    ...
    public static String findWinner(int n, int x, int y) {
        ...
        for (int i=2;i<=n;i++)
        {
            ...
            if (dp[i-1]==0)                //means just pehle wala step me nhi jeeta toh agle me jeetega
            {
                dp[i]=1;
            }
            else if ((i-x)>=0 && dp[i-x]==0) //so that no error occurs...agar pehle hara toh ab jeetega
            {
                dp[i]=1;
            }
            else if ((i-y)>=0 && dp[i-y]==0) //so that no error occurs...agar pehle hara toh ab jeetega
            {
                dp[i]=1;
            }
            else               //matlab pehle jeeta tha toh ap haarega
            {
                dp[i]=0;
            }
