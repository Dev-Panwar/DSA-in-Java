public class MinCostPath {
    public static int minCostPath(int arr[][]){   //recursive
    ...
    private static int minCostPath(int[][] arr, int i, int j) {
        ...
        if(i==m-1 && j==n-1){   //means reached the destination
            return arr[i][j];
        }

        if(i>=m || j>=n){          //if array reached a position that is not present.
    ...
    public static int minCostPathM(int arr[][]){    //memoization
        int m=arr.length; //row length
        int n=arr[0].length; //column length
    ...
    public static int minCostPathDP(int arr[][]){    //DP
        int m=arr.length; //row length
        int n=arr[0].length; //column length
        int storage[][]=new int[m][n];
        ...
        for (int i = m-2; i >=0; i--) {       
            for (int j = n-2 ; j>=0;j--) {
                 storage[i][j]=arr[i][j]+ Math.min(storage[i][j+1],Math.min(storage[i+1][j+1], storage[i+1][j]));          //same as Memoization and Recursion approach
            }
        }
        return storage[0][0];
    }
